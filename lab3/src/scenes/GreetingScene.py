...
class GreetingScene(BaseScene):
    ...
    def enter_as_user(self, answers):
        ...
        me = User.load(self.redis, login, self.driver)

        if not me:
            me = User(login, 'admin' if login.endswith('admin') else 'user', self.redis, self.driver)
            me.save()
        ...
        self.session['me'] = me
        UserActionsScene(self.session, self.redis, self.driver).enter()
